...
export function ProductsTable({ dataSource, onEdit, onDelete }) {
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    const pdfColumns = Product.columns.map(col => ({ header: col.title, dataKey: col.dataIndex }));
    ...
    })),
    {
      ...
      render: (_, record) => (
        <>
          ...
          <Button onClick={() => navigate(`/compras/productos/${record.id}`)}>Ver Detalles</Button>
        </>
      ),
    },
  ];
